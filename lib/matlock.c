...
dontkillme(void)
{
	...
	if (!(f = fopen(oomfile, "w"))) {
		if (errno == ENOENT)
			return;
		die("%s: fopen %s: %s\n", prog_name, oomfile, strerror(errno));
	}
	fprintf(f, "%d", OOM_SCORE_ADJ_MIN);
	if (fclose(f)) {
		if (errno == EACCES)
			die("%s: unable to disable OOM killer. "
...
gethash(void)
{
	...
	if (!strcmp(hash, "*")) {
		...
		if (!(pw = getpwuid_shadow(getuid())))
			die("%s: getpwnam_shadow: cannot retrieve shadow entry. "
			    "Make sure to suid or sgid %s.\n", prog_name, prog_name);
		hash = pw->pw_passwd;
#else
		die("%s: getpwuid: cannot retrieve shadow entry. "
		    "Make sure to suid or sgid %s.\n", prog_name, prog_name);
#endif /* __OpenBSD__ */
	}
#endif /* HAVE_SHADOW_H */
	...
	if (argc > 0) {
		...
		case 0:
			if (close(ConnectionNumber(dpy)) < 0)
				die("%s: close: %s\n", prog_name, strerror(errno));
			execvp(argv[0], argv);
			fprintf(stderr, "%s: execvp %s: %s\n", prog_name, argv[0], strerror(errno));
			_exit(1);
		}
	}
